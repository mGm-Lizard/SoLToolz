...
replication
{
    ...
    reliable if (Role < ROLE_Authority)
        ServerSetTradeMenu, ServerSetMuted, ServerSetLlama, ServerSendCustomMessage;
}
...
simulated function SendCustomMessage(byte MsgIndex)
{
    local PlayerController OwnPC;
    local SoLPlayerReplicationInfo OwnRI;
    ...
    if (OwnPC != None)
        OwnRI = class'SoLPlayerReplicationInfo'.static.GetFor(OwnPC.PlayerReplicationInfo);
    if (OwnPC != None && OwnRI != None)
        OwnRI.ServerSendCustomMessage(OwnPC, self, MsgIndex);
}

protected function ServerSendCustomMessage(PlayerController Sender, SoLPlayerReplicationInfo Context, byte MsgIndex)
{
    local SoLPlayerReplicationInfo SenderRI;
    ...
    if (Context != self)
    {
        Context.ServerSendCustomMessage(Sender, Context, MsgIndex);
        return;
    }
    ...
    {
        SenderRI = class'SoLPlayerReplicationInfo'.static.GetFor(Sender.PlayerReplicationInfo);
        if (SenderRI == None || !SenderRI.bTradeMenu)
            return;
    }

    PlayerController(Owner).ReceiveLocalizedMessage(class'SoLMessages', MsgIndex + 10);
}
